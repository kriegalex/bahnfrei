...
| TASK-052 | Post-tag quick wins per DEC-032: timing-conflict merge becomes fill-blanks-only — the imported mark is preserved and only blank fields taken from the counterpart, matching the documented intent (OQ-062); roster-page out-of-competition toggle over the existing complete service layer (`SetOutOfCompetition`, OQ-091); letter-marker quick-action buttons hidden above the phone breakpoint, values-only CSS (OQ-133); complete the TASK-047 empty-state sweep across seeding, reconciliation, officials and timing-import surfaces (OQ-135) | SYS-152, SYS-147, SYS-116; DEC-032 | 0.1 tag | Sonnet | S |
| TASK-053 | Admin-issued password reset per DEC-030 (OQ-116): organizer/admin sets a one-time temporary password for any account (forced change on next login), audited like other account mutations; no email infrastructure (offline-venue posture); runbook account-recovery section rewritten from "recreate the account" to this flow; DE/FR | SYS-090/091; DEC-030 | 0.1 tag | Sonnet | S |
