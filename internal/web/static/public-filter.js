...
(function () {
    ...
    function applyFilter(state) {
        ...
        let visible = 0;
        const rows = state.root.querySelectorAll("[data-filter-row]");
        ...
        const sections = state.root.querySelectorAll("[data-filter-section]");
        sections.forEach(function (section) {
            const allRows = section.querySelectorAll("[data-filter-row]");
            if (allRows.length === 0) {
                return; // nothing to filter in this section — leave it exactly as rendered
            }
            const hasVisible = section.querySelector("[data-filter-row]:not([hidden])");
    ...
    function bind(root, initialQuery) {
    ...
    document.querySelectorAll("[data-public-filter]").forEach(function (root) {
        ...
        const liveSection = root.closest("[data-sse-refresh]");
        if (!liveSection) {
            return;
        }
        const observer = new MutationObserver(function () {
            const freshRoot = liveSection.querySelector("[data-public-filter]");
            if (!freshRoot) {
                return;
            }
