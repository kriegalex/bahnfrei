...
| OQ-021 *(TASK-010, 2026-07-10)* | **Start-list publication step:** SYS-070 lists "start lists" among the public surfaces and SYS-071 gives publications a 60 s budget, implying a deliberate publish action — but only the timetable has a versioned draft/published state today (`TimetableVersion`); the domain model's `OfficialDocument.DocumentStartList` exists but is unwired. PoC behaviour: `GET /m/{id}/startlists` serves the live roster directly (see `internal/web/public.go`). Decide whether start lists need their own published snapshot (timetable-style) once seeding lands (TASK-018) — if so, that is an ADR-worthy publication-model decision, not a silent change. | Tech lead (revisit at TASK-018) | No — PoC surfaces are labeled unofficial (SYS-076) and UKC has no seeded heats |
| OQ-022 *(TASK-010, 2026-07-10)* | **Discipline-catalog localization coverage (SYS-074):** public pages localize discipline names via `discipline.<code>` i18n keys with fallback to the catalog's canonical English name; keys are shipped only for the disciplines the built-in UKC template and test fixtures exercise, not the full `catalog.json`. Complete the DE/FR catalog pass in TASK-024 (UC-025 full i18n completion). | Tech lead (fold into TASK-024) | No — fallback renders a readable English name, never a raw key |
