...
| TASK-038 ✅ | **Done.** Entry/roster search per DEC-021 (OQ-067): a server-side `q` query-param filter (case-insensitive name/bib/club substring match, `app.MatchesParticipantSearch`) on the roster (`/meets/{id}/roster`) and bib-assignment (`/meets/{id}/bibs`) operator lists, DE/FR labels, a plain keyboard-reachable GET form. The reserved SYS-120 benchmark sub-test landed alongside the existing full-list-load proxy: `TestSYS120ReferenceScaleOperatorBudgets/ParticipantSearch` (real filtered search, reference scale). See `docs/requirements/traceability-matrix.md` (SYS-120 row) and `docs/delivery/perf-and-recovery-task-027.md` for measured numbers. | SYS-120, SYS-114 | M2 | Sonnet | S |
