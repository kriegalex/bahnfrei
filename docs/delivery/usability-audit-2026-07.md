...
| Findings | Count |
|---|---|
| Critical | 2 (OQ-074, OQ-075) — **both closed by TASK-034** |
| Major | 1 (OQ-076) |
| Minor | 1 (OQ-077) |
...
check, gallery, checklist) — both were cross-cutting handler/interaction changes (a confirmation
mechanism; per-field error attribution), not design-token issues, so they were recorded as open
questions (OQ-074, OQ-075) rather than fixed in TASK-032 itself. **Status update (TASK-034,
2026-07-14): both closed.** OQ-074's destructive-action confirmation flow
(`internal/web/confirm.go`/`.templ`) and OQ-075's inline field-error mechanism
(`internal/web/fielderrors.go`/`.templ`, adopted on meet setup/online entry/result correction)
now satisfy UC-038 #3's "zero open critical findings" precondition ahead of the release-0.1
audit re-run (TASK-028). This run does not itself certify a release; it establishes the checklist
and its baseline.
...
| H3 | User control and freedom | **Closed (TASK-034, OQ-074)** | Erase/disable/archive/purge each now require a GET confirm sub-page (`internal/web/confirm.go`/`.templ`); erasure and retention purge additionally require typing a confirmation token. See OQ-074's closed status for the full mechanism. |
...
| H9 | Help users recognize/diagnose/recover from errors | **Closed (TASK-034, OQ-075)** | The three representative forms (meet setup, online entry, result correction) now render an inline `.field-error` naming the offending field and what to fix, via the reusable `FieldErrors` mechanism (`internal/web/fielderrors.go`); input remains preserved. See OQ-075's closed status. |
...
| F3 | Inline validation error | **Closed (TASK-034, OQ-075)** | Meet setup, online entry and result correction now render `aria-invalid`/`.field-error` at the offending input (`TestMeetCreateFieldErrorsOQ075UC038_4`, `TestOnlineEntryIndividualFieldErrorsOQ075UC038_4`, `TestCaptureAnnounceAndCorrectionFlowSYS046SYS047UC015Web`'s no-reason case); the page-level flash remains as a summary. |
...
| F5 | Destructive-action confirmation | **Closed (TASK-034, OQ-074)** | Same fix as H3. |
...
**Left open at the time** (OQ-074…OQ-077): the confirmation mechanism, per-field error
attribution, and hint-text content pass were all real product changes across many existing
handlers/templates — outside a design-system-consolidation task's scope. **OQ-074 and OQ-075
closed in TASK-034**; OQ-076 (hint-text pass) substantially closed in TASK-031; OQ-077 (progress
feedback) remains open (Minor, low risk at current scale).
...
representative form gets a real error case, and confirm the closed findings (OQ-074, OQ-075,
OQ-076) hold up under the fuller fixture before declaring zero open Critical findings.
