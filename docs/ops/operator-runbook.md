...
**Audience:** the club member who runs the instance — before, during, and after a meet.
...
There is no self-registration for operator roles by design — hand out credentials
...
non-goal** — nobody at this club tier builds it; tolerating connectivity gaps instead of
preventing them is the answer for this release. Practically, that means:
...
- **Every operator device needs the hub's address once**, e.g. `https://<laptop-ip>:8443` in venue
  mode (self-signed certificate — accept the one-time browser warning per device) or the
...
- **Capture continues through connectivity blips.** A device that has loaded its assigned event
  ...
  idempotently on reconnection — no manual "sync" button, no duplicates. The authenticated shell
  shows a degraded-connectivity banner while a blip is active; it clears on its own on reconnect.
...
## 3. Timing-agent mode ([ADR-006](../architecture/adr/ADR-006-timing-integration.md))
...
3. The agent regenerates `lynx.ppl`/`.sch`/`.evt` in the watch directory whenever the hub's start
   ...
   meet's timing page for an operator to resolve.
4. Manual capture (the office/infield UI) remains a co-equal path at all times, not a fallback:
   ...
   FAT) is always recorded.

No timing hardware yet, or a non-Lynx system (ALGE, TimeTronics)? The meet's timing page also
offers documented generic CSV import/export through the same conflict-resolution pipeline.
...
**Restore** onto a fresh installation — restore never merges into an existing database:

```
bahnfrei restore --data-dir ./restored-data --from ./backups/meet-2026-07-14.bfbak
```

Opening the restored database re-runs the startup consistency check, so a restore is *verified*,
not just copied — it reports the meet count and confirms the checksum
before you point `bahnfrei serve` at the restored directory. Take a backup at least once per
competition day (more often for a multi-day meet) and after the meet closes; the documented
restore-time target is a full-fidelity restore in ≤15 minutes, which the CLI round-trip above
comfortably meets on commodity hardware.
...
live-results viewer capacity (the 2,000-concurrent-viewer target is met); see
`docs/ops/defect-policy.md` for how defects are triaged and what "no known critical/high defects
at release" means in practice.
