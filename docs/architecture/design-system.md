...
| Form layout | `.field` (one label+input block, plus its paired hint/error/help-icon), `.form-grid` (a form's fields, consistent spacing), `.form-row` (fields laid out side by side within a `.form-grid`), `.form-panel` (bounded, lightly bordered, centered card on `--color-surface`) | default only | DEC-037, TASK-057: a stacked label-above-input convention so a label never ends one line while its input starts the next — `.field` wraps an EXISTING `<label>…<input/></label>` (implicit label association is preserved, no new `for`/`id` plumbing). Applied to login/first-run setup/change-password (`.form-panel`, narrow and centered), the admin account-creation form, the create/edit meet form, and the seeding generate/advance forms (`.form-row`, multiple short fields side by side). DEC-038, TASK-058 completed the sweep (closing OQ-153): the entries individual form, the relay entry form's leg/reserve composition rows (`.form-row`, mirroring the meet form's session rows), the timing-import form and the entries-import form. Deliberately **excluded**, matching the capture-grid rationale below: the bulk club-entry table's per-participant grid and the relay-composition edit sub-form (both dense, scannable multi-row data entry — stacking every cell would defeat the table); the timing-agents create form and the roster/bibs search forms (genuinely one control row); the reconciliation and officials-assignment pages, whose only forms are button-only apply/discard/assign actions with no labeled field to stack. |
| Checkbox grid | `.checkbox-grid` (wraps a `fieldset`'s checkboxes) | default, focus-visible (ring, inherited from the generic checkbox rule), disabled | DEC-038, TASK-058: replaces the discipline-add form's `<select multiple>` category listbox with one checkbox per category, wrapping compactly instead of stacking one per line. Every checkbox keeps the SAME `name="categories"` the listbox submitted under — Go's `r.Form["categories"]` reads a checkbox group and a multi-select identically, including the zero-selected case (`AddEvent` still rejects it with "at least one category is required", SYS-002) — so the handler, its validation and its tests are unchanged. `meets.templ`'s programme-add form; documented live on the gallery page reusing the existing fieldset fixture. |
