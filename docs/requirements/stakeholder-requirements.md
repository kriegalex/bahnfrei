...
| STR-044 | Wherever the system asks for input whose meaning, format, or consequence is not self-evident to a first-time user (anything beyond universally familiar fields such as a name, e-mail address, or password), a short explanation SHALL be available at the point of use — visible on-screen or one deliberate action away — without leaving the screen or consulting external documentation. | Volunteer operators meet domain jargon (wind reading, countback, seeding method) once a year; point-of-use help removes the "call the one person who knows" dependency and reinforces STR-035. Information *required* to complete a task must stay visible, not hidden behind help (NN/g tooltip guideline #1). | Founder (request 2026-07-13); SH-SEC, SH-ORG, SH-CLB | MVP |
| STR-045 | All user-facing surfaces SHALL present a coherent, contemporary visual and interaction design of professional quality, achieved by verifiable conformance to a documented design system and to published authoritative UX guidance — not by ad-hoc styling. ("Modern design" is operationalized as measurable system requirements: SYS-116 design system, SYS-117 interaction conventions and per-release usability audit.) | Founder seed asks for "state of the art, modern"; adjectives are not verifiable, so this StR binds the intent to measurable SyRS targets. A trustworthy public face also matters for adoption against incumbent tooling. | Founder seed; Founder (request 2026-07-13); SH-SPE, SH-ORG | MVP |
| STR-046 | A volunteer official working at the point of competition SHALL be able to perform their assigned tasks reliably on their own smartphone under field conditions — small screen, touch-only input, bright outdoor light, one-handed use, intermittent attention — and SHALL at all times be able to tell whether captured data has reached the system and what to do when an entry was not accepted. | Field capture runs on officials' own phones over mobile data (C8; ADR-002 makes field-wide venue Wi-Fi a non-goal). Existing usability StRs cover learning effort (STR-035), point-of-use help (STR-044) and public surfaces (STR-034), but none binds the operator experience to the volunteer's actual device and environment; the 2026-08 volunteer walkthrough found the primary capture surface unusable without horizontal panning at phone width and sync rejections misreported as connectivity loss (F1/F2, `../delivery/usability-audit-volunteer-2026-08.md`). | Volunteer-persona walkthrough 2026-08-06 (*proposed — pending founder ratification*); SH-SEC, SH-ORG, SH-CLB; C8 | MVP |
