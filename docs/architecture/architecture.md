...
## 8. Library selections (pre-approved, non-ADR)

Library choices within an already-ratified module (`pdf/`, table above) that do not change
the stack, data model or an external integration are recorded here rather than as a new ADR
(work-breakdown TASK-011 note).

**PDF generation (TASK-011, SYS-072, PoC scope): `codeberg.org/go-pdf/fpdf`.**

- **What it is:** a pure-Go PDF document generator (text, tables, drawing, images) — the
  actively maintained continuation of the archived `github.com/jung-kurt/gofpdf` (moved to
  Codeberg after its GitHub mirror was archived 2025-03-04; 657+ commits, 16 tags, latest
  release v0.12.0, last push 2026-05-27).
- **Licence:** MIT (verified in the module's `LICENSE` file). fpdf's own `go.mod` declares
  further dependencies for optional `contrib/` subpackages (barcode/PDF417 generation,
  image handling) we do not import — `go mod why` confirms our build pulls in none of them;
  the core package we use has no further runtime dependency at all. Were that to change,
  those modules (`boombuler/barcode`, `phpdave11/gofpdi`, `ruudk/golang-pdf417`,
  `golang.org/x/image`, `pkg/errors`) are themselves MIT/BSD — permissive and compatible
  with AGPL-3.0-only distribution (ADR-001). MIT permits static linking into an AGPL binary
  without relicensing fpdf.
- **Why not the alternatives:** `github.com/unidoc/unipdf` is dual-licensed
  (AGPL-3.0/commercial) with usage terms oriented at a paid tier — avoidable license
  friction for no PoC benefit. Full LaTeX/wkhtmltopdf/headless-Chromium approaches add a
  non-Go runtime dependency, at odds with the single-binary deployment model (ADR-002/003).
  `maroto`/`gopdf`-style higher-level layout libraries wrap the same low-level primitives
  fpdf provides directly; the PoC's table/grid layouts don't need that abstraction.
- **How it's used:** `internal/pdf` wraps fpdf behind a small generic `Document`/`Header`/
  `Section` model (no bahnfrei domain type crosses the package boundary) — core Helvetica
  font with the `UnicodeTranslatorFromDescriptor("")` (cp1252) transliteration, which covers
  German/French diacritics without embedding font files.
- **Verification:** because fpdf has no built-in "read this back" API, tests parse the
  generated bytes with a second, independent library — `github.com/ledongthuc/pdf`
  (BSD-3-Clause, a maintained fork of `rsc.io/pdf` that adds plain-text extraction) — so
  assertions check real PDF structure/content rather than merely non-empty output. This is a
  test-only dependency; also permissively licensed.
